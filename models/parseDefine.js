...
function getIntData(key,arrRange,data){
    var ret = {};
    var start = arrRange[0];
    var end = arrRange[1];
    var diff = arrRange[2];
    var newData = data.substring(start,end);
    
    if(key === 'cond'){
        
        var t1 = parseInt(data.substring(0,2),16);
        var intData = parseInt(newData.substring(2,6),16);
        if(t1 === 2)
            diff = 1000;
        else
            diff = 100;
    }else{
        var intData = parseInt(newData,16);
    }
    console.log( key+'  : '+newData +' => '+intData/diff);
    return intData/diff;
}